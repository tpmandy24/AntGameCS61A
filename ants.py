...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            ant_already_in_place = place.ant
            if ant_already_in_place.can_contain(self):
                ant_already_in_place.store_ant(self)
            elif self.is_container and self.can_contain(ant_already_in_place):
                self.store_ant(ant_already_in_place)
                place.ant = self
            else:
    ...
    def double(self):
        ...
        """if hasattr(self, 'buffed'):
            if not self.buffed:
                self.damage *= 2
                self.buffed = True"""
        if self not in QueenAnt.doubled_ants:
            self.damage *= 2
        QueenAnt.doubled_ants.append(self)
        # END Problem 12
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        """check = self.place
        while check.is_hive == False:
            if random_bee(check.bees) != None:
                return random_bee(check.bees)
    ...
    def action(self, gamestate):
        ...
        """nearest_bee = self.nearest_bee()
        if nearest_bee is not None:
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.turns_to_chew > 0:
            ...
            bees = self.place.bees[:]  # Make a copy of the bees list
            if bees:
...
class TankAnt(ContainerAnt):
    """TankAnt that protects an ant in its place and deals 1 damage to all bees in its place each turn."""
    name = 'Tank'
    implemented = True
    food_cost = 6
    damage = 1
    ...
    def action(self, gamestate):
        # Deal 1 damage to all bees in the ant's place
        for bee in self.place.bees[:]:
            bee.reduce_health(self.damage)

        # Allow the contained ant to perform its action
        if self.ant_contained is not None:
            self.ant_contained.action(gamestate)
# END Problem 9
...
class ScubaThrower(ThrowerAnt):
    """ThrowerAnt that is waterproof and can be placed in Water."""
    name = 'Scuba'
    implemented = True
    food_cost = 6
    is_waterproof = True
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        pexit = self.place.exit
        while pexit:
            if pexit.ant:
                if isinstance(pexit.ant, ContainerAnt) and pexit.ant.ant_contained:
                    pexit.ant.ant_contained.double()
                pexit.ant.double()
            pexit = pexit.exit
        
        """self.double_ants_in_tunnel(gamestate)"""
    ...
    def double_ants_in_tunnel(self, gamestate):
        """Double the damage of all ants behind the QueenAnt in the tunnel."""
        place = self.place
        while place is not None:
            if place.ant is not None:
                self.double_ant_damage(place.ant)
            place = place.exit
    def double_ant_damage(self, ant):
        """Double the damage of the given ant and any ants it contains."""
        if not hasattr(ant, 'buffed'):
            ant.buffed = False
        if not ant.buffed:
            ant.damage *= 2
            ant.buffed = True
    # If the ant is a container ant, double the damage of the contained ant as well
        if ant.is_container and ant.ant_contained is not None:
            self.double_ant_damage(ant.ant_contained)
############
# Optional #
############
